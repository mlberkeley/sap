...
def running_averages(D_F_Fprev_R, A_0, B_0, eta):
    ...
    df = df.withColumn('R_R2_weight', eta * F.pow(1 - eta, N - F.col('index')))
    df = df.withColumn('forw_weight', F.pow(1 - eta, F.col('index')))
    df = df.withColumn('rev_weight', F.pow(1 - eta, -(N - F.col('index'))))

    # Find (1 - eta)^(-(N - i)) to reverse the decay after the scan. Finds the decayed versions of A_0, B_0, R, R^2
    df = df.withColumn('A_0_weighted', F.col('forw_weight') * A_0)
    df = df.withColumn('B_0_weighted', F.col('forw_weight') * B_0)
    df = df.withColumn('R_weighted', F.col('R_R2_weight') * F.col('R'))
    df = df.withColumn('R2_weighted', F.col('R_R2_weight') * F.col('R2'))
    ...
    df = df.withColumn('A', F.col('A_0_weighted') + F.col('rev_weight') * F.sum('R_weighted').over(w_scan))
    df = df.withColumn('B', F.col('B_0_weighted') + F.col('rev_weight') * F.sum('R2_weighted').over(w_scan))
    ...
    # Calculate dU_dR
    df = df.withColumn('dU_dR', (F.col('B') - F.col('A') * F.col('R')) / F.pow(F.col('B') - F.col('A') * F.col('A'), 1.5))
